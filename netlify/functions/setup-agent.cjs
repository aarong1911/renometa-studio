...
exports.handler = async function (event, context) {
  ...
  if (recentRequests && recentRequests.length >= MAX_WEBSITES_PER_USER) {
    const oldest = recentRequests.sort(
  ...
  try {
    ...
    if (error) {
      ...
      if (error.code === "23505") {
        return {
          statusCode: 409,
          headers: corsHeaders,
          body: "Agent request with this ID already exists.",
        };
      }
      return {
        statusCode: 500,
    ...
    try {
      ...
      if (crawlResponse.status !== 202 && crawlResponse.status !== 200) {
        ...
        console.error(
          `Unexpected status from background function: ${crawlResponse.status} - ${responseText}`,
        );
